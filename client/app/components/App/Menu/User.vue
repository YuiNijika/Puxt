...
<template>
    <UDropdownMenu v-if="authStore?.isAuthenticated" :items="items" :ui="{ content: 'w-48' }">
        <UAvatar :src="userInfo?.avatar || '/images/Azusa.jpg'" :alt="userInfo?.name || 'User'"
            :class="{ 'opacity-50': userInfoLoading }" @click="handleAvatarClick" />
        <template v-if="userInfoLoading" #trigger>
            <UAvatar :src="userInfo?.avatar || '/images/Azusa.jpg'" :alt="userInfo?.name || 'User'" class="opacity-50">
                <template #badge>
